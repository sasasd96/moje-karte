...
function s.costfilter(c)
	return c:IsLevelBelow(6) and c:IsRace(RACE_SPELLCASTER) and aux.IsCodeListed(c,46986414) and c:IsReleasable()
end
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() then
		local ct=Duel.GetMatchingGroupCount(aux.FilterBoolFunctionEx(Card.IsRace,RACE_SPELLCASTER),tp,LOCATION_GRAVE,0,nil)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
