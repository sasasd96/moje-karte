...
function s.initial_effect(c)
	...
	--Must be Special Summoned with the effect of "Steel Lamp"
	...
	c:RegisterEffect(e3)
	--If banished: Activate Macro Cosmos
...
s.listed_names={7,30241314} --Chaos Distill, Macro Cosmos
s.listed_series={0x501} --Alchemy Beast

function s.splimit(e,se,sp,st)
	return se:GetHandler():IsCode(12) --Steel Lamp
end
...
function s.actfilter(c,tp)
	return c:IsCode(30241314) and (c:IsLocation(LOCATION_HAND) or c:IsLocation(LOCATION_DECK)) and c:GetActivateEffect():IsActivatable(tp,true)
end
